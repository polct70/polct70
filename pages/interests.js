...
export default function InterestsPage() {
  ...
  async function fetchItems() {
    const { data } = await supabase.from('items').select('*').eq('type', 'interest').order('created_at', { ascending: false });
    setItems(data || []);
  }
  ...
  async function addItem() {
    ...
    await supabase.from('items').insert([{ owner: user.id, type: 'interest', title: null, content }]);
    setContent('');
    fetchItems();
  }
  ...
  return (
    <div>
      ...
      <main className="max-w-3xl mx-auto py-8 px-6">
        <h2 className="text-2xl mb-4">兴趣</h2>
